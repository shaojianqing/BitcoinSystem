...
public class Bech32Utils {
    ...
    public static class Bech32Bytes {
         ...
         */
        public static Bech32Bytes ofBytes(byte[] data) {
         ...
         * @return Bech32 instance containing 5-bit encoding
         */
        public static Bech32Bytes ofSegwit(short witnessVersion, byte[] witnessProgram) {
     ...
     * @throws AddressException if invalid
     */
    private static Encoding verifyChecksum(final String hrp, final byte[] values) {
        ...
        if (check == BECH32_CONST)
            ...
            throw new AddressException("The checksum is not valid!");
    }
     ...
     * @throws AddressException if unexpected hrp or encoding
     */
    public static byte[] decodeBytes(String bech32, String expectedHrp, Encoding expectedEncoding) {
        ...
        if (!decoded.hrp.equals(expectedHrp) || decoded.encoding != expectedEncoding) {
            throw new AddressException("unexpected hrp or encoding");
        }
    ...
    public static String encode(Encoding encoding, String hrp, Bech32Bytes values) {
        AssertUtils.isTrue(hrp.length() >= 1, "human-readable part is too short: " + hrp.length());
        AssertUtils.isTrue(hrp.length() <= 83, "human-readable part is too long: " + hrp.length());
     ...
     * @throws AddressException if the string is invalid
     */
    public static Bech32Data decode(final String str) throws AddressException {
        boolean lower = false, upper = false;
        if (str.length() < 8)
            throw new AddressException("Input too short: " + str.length());
        if (str.length() > 90)
            throw new AddressException("Input too long: " + str.length());
        for (int i = 0; i < str.length(); ++i) {
            char c = str.charAt(i);
            if (c < 33 || c > 126) throw new AddressException("invalid character:" + c);
            if (c >= 'a' && c <= 'z') {
                if (upper)
                    throw new AddressException("invalid character:" + c);
                lower = true;
            }
            if (c >= 'A' && c <= 'Z') {
                if (lower)
        ...
        if (pos < 1) throw new AddressException("Missing human-readable part");
        final int dataPartLength = str.length() - 1 - pos;
        if (dataPartLength < 6) throw new AddressException("Data part too short: " + dataPartLength);
        byte[] values = new byte[dataPartLength];
        for (int i = 0; i < dataPartLength; ++i) {
            char c = str.charAt(i + pos + 1);
            if (CHARSET_REV[c] == -1) throw new AddressException("invalid character:" + c);
    ...
    private static byte[] convertBits(final byte[] in, final int inStart, final int inLen, final int fromBits,
                                      final int toBits, final boolean pad) throws AddressException {
        int acc = 0;
        int bits = 0;
        ...
        for (int i = 0; i < inLen; i++) {
            ...
            if ((value >>> fromBits) != 0) {
                throw new AddressException(String.format("Input value '%X' exceeds '%d' bit size", value, fromBits));
            }
        ...
        if (pad) {
            ...
            throw new AddressException("Could not convert bits, invalid padding");
        }
